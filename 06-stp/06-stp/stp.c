...
// *p1 > *p2, return 1; *p1 = *p2, return 0; *p1 < *p2, return -1; 
int config_cmp_port_port(stp_port_t *p1, stp_port_t *p2){

	if(p1->designated_root != p2->designated_root)
		return (p1->designated_root > p2->designated_root)? -1:1;
	else if(p1->designated_cost != p2->designated_cost)
		return (p1->designated_cost > p2->designated_cost)? -1:1;
	else if(get_switch_id(p1->designated_switch) != get_switch_id(p2->designated_switch))
		return (get_switch_id(p1->designated_switch) > get_switch_id(p2->designated_switch))? -1:1;
	else if(get_port_id(p1->designated_port) != get_port_id(p2->designated_port))
		return (get_port_id(p1->designated_port) > get_port_id(p2->designated_port))? -1:1;
	else
		return 0;
}

//update the port to undesigned port
void undesigne_port(stp_port_t *p, struct stp_config *config) {
...
static void stp_update(stp_t *stp, struct stp_config *config) {
	...
	for (i++ ; i < stp->nports ; i++) {
		if(	root_port!=NULL && !stp_port_is_designated(&stp->ports[i]) && config_cmp_port_port(root_port, &stp->ports[i]) == -1)
			root_port = &stp->ports[i];
	}
	...
	for (i = 0; i != stp->nports; i++ ) {
		if(!stp_port_is_designated(&stp->ports[i]) && stp->root_path_cost < stp->ports[i].designated_cost) {
